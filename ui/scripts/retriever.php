...
class DatabaseRetriever {
    ...
    private function generateRelevancyTree($article, $maxNodesAtDepth, $maxDepth) {
        ...
        for ($d = 0; $d < $maxDepth; $d++) {
            ...
            if ($sz > 0) {
                $querystring = "SELECT * FROM ArticleRelations WHERE Strength != -1 AND ( Article = '" . mysql_real_escape_string($names[0]) . "'";
                for ($i = 1; $i < $sz; ++$i)
                    ...
                    if ($row['Strength'] != -1) {
                        ...
                        if (!in_array(strtolower($childn), $articlesUsed)) {
                            ...
                            if (sizeof($currentDepth[$parentname]->children) < $maxNodes) {
                                ...
                                if ($row['Strength'] == 0)
                                    $strength = $this->calculateStrength($parentname, $childn);
